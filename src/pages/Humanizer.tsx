...
const Humanizer = () => {
  ...
  const [tone, setTone] = useState<"formal" | "informal" | "interview" | "explanatory">("informal");
    ...
    <div className="space-y-6 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500">
          Transform robotic AI text into undetectable, human-sounding narrative using the Alex Rivera engine.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-200px)] min-h-[500px]">
        ...
        <Card className="p-4 flex flex-col gap-4 neo-shadow border-2 border-black">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs font-mono ${input.length > 3000 ? "text-red-500" : "text-slate-400"}`}>
              {input.length}/3000
            </span>
          </div>
        ...
        <Card className="p-4 flex flex-col gap-4 neo-shadow border-2 border-black bg-slate-50">
          ...
          <div className="flex items-start gap-2 text-[10px] text-slate-400 p-2 bg-yellow-50/50 border border-yellow-100 rounded">
            ...
            <p>Output generated by prompt injection engine. Results vary based on input complexity.</p>
          </div>
        </Card>
      </div>
    </div>
